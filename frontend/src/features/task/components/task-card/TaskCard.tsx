...
export const TaskCard = ({ task, onEdit }: TaskCardProps) => {
  ...
  return (
    <div className="group relative rounded-lg border border-gray-200 bg-white p-4 shadow-sm transition-shadow duration-200 hover:shadow-md dark:border-slate-700 dark:bg-slate-800">
      <div className="mb-3 flex items-start justify-between">
        <div className="flex items-center gap-2">
          <span className="text-lg">{statusIcons[task.status]}</span>
          <span
            className={`rounded-full border px-2 py-1 text-xs font-medium ${statusColors[task.status]}`}
          >
            {task.status.replace("-", " ")}
          </span>
        </div>
          ...
          {showActions && (
            <div className="absolute top-8 right-0 z-10 min-w-[120px] rounded-lg border border-gray-200 bg-white py-1 shadow-lg dark:border-slate-600 dark:bg-slate-800">
              <button
                ...
                className="flex w-full items-center gap-2 px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 dark:text-gray-300 dark:hover:bg-slate-700"
              >
      ...
      {task.extras && Object.keys(task.extras).length > 0 && (
        <div className="mb-3 flex flex-wrap gap-2">
          {task.extras.priority && (
            <span
              className={`rounded-full px-2 py-1 text-xs font-medium ${getPriorityColor(task.extras.priority)}`}
            >
